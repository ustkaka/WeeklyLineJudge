...
def getKDJTrend(stockStat, M15_NO):
    kdjTrend        = 0
    if stockStat.kdjk.values[M15_NO] < 30:
        kdjTrend    = 1
    elif stockStat.kdjk.values[M15_NO] > 70:
        kdjTrend    = -1
...
def getMACDTrend(stockStat, M15_NO):
    macdTrend       = 0
    if stockStat.macdh.values[M15_NO] > stockStat.macdh.values[M15_NO - 1] > stockStat.macdh.values[M15_NO - 2] > 0:
        macdTrend   = 1
    elif stockStat.macdh.values[M15_NO] < stockStat.macdh.values[M15_NO - 1] < stockStat.macdh.values[M15_NO - 2] < 0:
        macdTrend   = -1
    
    return macdTrend

# 5、K线突破函数： getKBreakTrend()#############################################
# 以K线突破函数判断15分钟线第M15_NO个节点的多空， kBreakTrend = -1表示指标空，=1表示指标多，=0表示无法进行多空判断
...
# 以K线反转多函数判断15分钟线第M15_NO个节点的多空， getKReverseBull =1表示指标多，=0表示得不到“多”的结论。
...
# 以K线反转空函数判断15分钟线第M15_NO个节点的多空， getKReverseBear =1表示指标空，=0表示得不到“空”的结论。
    ...
    for m15_no in m15_no_list:#本例子中，第6个节点前不包括完整的波峰、波谷，所以从第6周开始计算。M15_NO为当前时间节点
        ...
        if weekTrend in (0,10) and latestPrice <= mean_3weeks:#(3) W(D)=0,10
            if kBreakTrend == 1:            #K线突破多
                ...
                ship        = capital * shiprate//order_p
                order(code, order_a, current_k, order_p, ship, target, stop, retreat_k)
        if weekTrend in (0,10) and latestPrice >= mean_3weeks:#(4) W(D)=0,10
            if kBreakTrend == -1:           #K线突破空
                order_a     = "sell"
                order_p     = quote_g15m[1] - 2*m15_tick
                ship        = capital * shiprate//order_p
                ...
                ship        = capital * shiprate//order_p
                order(code, order_a, current_k, order_p, ship, target, stop, retreat_k)
np.savetxt(code+'_order.csv', trade_records, delimiter=',', fmt='%s')
########################################## 六、交易操作 ########################################
###############################################################################################
#期货代码，挂单操作，挂单时间，  挂单开仓价， 挂单开仓量；止盈，     止损，  撤单时间，   实际操作，开仓时间，  开仓价格，  平仓时间，平仓价格， 利润
#[[code,  order_a, order_k,    order_p,    ship,      target,    stop ,  retreat_k ,  action ,  open_k ,  open_p ,   close_k , close_p , profit] ]
    ...
    for i in range(len(trade_records)):
        ...
        if action == 'buy':#情况2：该挂单已开仓，未平仓。假设挂单都能按指令价格止损、止盈
            ...
            if  stockStat.high.values[m15_no] <= open_p - target:#触动止盈操作，进行平仓
                ...
                trade_record[13]    = - ship * stop 
                trade_records[i]    = trade_record #更新trade_records数组中第i条挂单记录
np.savetxt(code+'_action.csv', trade_records, delimiter=',', fmt='%s')